...
## How it works
The external device can generate and store a keypair offline, the private key has thus never been known outside the device. A software wallet can connect to the signing device and act as an interface, or handle transactiosn airgapped from the device. 
