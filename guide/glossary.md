...
A piece of software that runs, and connects to, the Bitcoin network. 

### Bitcoin Core

An open source Bitcoin project that maintains and releases the primary Bitcoin reference implementation [Bitcoin Core client](bitcoin-core-client).
...
To fully verify that all transactions on the bitcoin network are valid, a [full node](#full-node) needs to download and examine all previous block-data. This is called the initial block download, or IBD. This can take several hours to days depending on the users hardware and only after it is complete can wallets linked to the node show up to date transasction history.
...
### Keys
Bitcoin uses public-key cryptography. Public-key cryptography uses randomly generated pair of keys comprising of a [public](#public-key) (which may be known to others) and [private key](#private-keys) (which should be kept secret from anyone but the owner).

**[Private key](https://en.bitcoin.it/wiki/Private_key)**<br/>
...
**Extended private key (xpriv)**<br/>

In hierarchical deterministic (HD) wallets, all a wallets private keys are derived from this XPRIV.

`xprv9zrji5mK3nb4RbuR2ZYFtyzK3gn78KnEzkNP4ZxwwPPwcgQQVZqnjTMAGxmmM3jpmfsthQUtfD9iYPvnaqwejCjcyEswLqEhX4LPKNFUXT5`

**Public key**<br/>

Bitcoin public keys are derived from private keys. Public keys can not derive private keys however. A Bitcoin addresses(#address) is generated by hashing a public key.

`02f062b2649e0665f2e8e5573172ef85921fc4b307e9795113930ed2229d271f4e`

**Extended public key (xpub)**<br/>

In hierarchical deterministic (HD) wallets, all addresses and their matching public keys are derived from this XPRIV. All public keys and subsequent addresses (by hashing these public keys) are generated from it.

`xpub6DE1bvvTaMkDaAghedChtufg3rDPeYdWt9sM5iTwBVYe9X6bmLenQrSexSa1qDscYtidSMUEo9u7TuXg48Y3hBXQr7Yw8zUaEToH1rVgvif`

**ypub** - A xpub belonging to a wallet deriving P2WPKH in P2SH addresses.

`ypub6Y4GubbNj3HhRTspUyzL6zmBDpMqbAd1oGPZs7MpZVvXCcuq1zpM2v6nyeXbq8XXxXqSBq4oFpFfMC9Empx4VRD1iTFMiuJ4WBrvQNtKV6V`

**Ypub** - A xpub belonging to a wallet deriving P2WSH in P2SH addresses. 

`Ypub6ixN2qKpHzr4r33CReTJw56ywcQ6oXJc7Y3EnNdMwGkvpoUjnQCd82xinMV5PZkSBzuR4R4v92eAUMm15471duKgYvMm8Jm4Pv8nzKXAkjA`

**zpub** - A xpub belonging to a wallet deriving P2WPKH addresses.

`zpub6rtYDGGHsiqBGm4wKLmxK5rgPnWHXncWiNuneWFhwWJQFij4GeyueykvzrVBq3BTNAxEwJfMiUcDEUkoVXN5HetcanwnJp7YmuvZnvQ9BU6`

**Zpub** - A xpub belonging to a wallet deriving P2WSH addresses.

`Zpub73ndLVzjSgPYhLEKG1Ew9ACV7aYYk9J72eZTZmXFKH8osuHy34NBk6croZSfPUQMbe2DotfUbgziMeNZnkX2S91HRG4BiDaYfeCSNwJZJ7n`

**Hierarchical Deterministic (HD) Wallet Keys**<br/>


**[Hardened extended keys](https://developer.bitcoin.org/devguide/wallets.html#hardened-keys)**<br/>


**[Loose-Keys - (JBOK)](https://developer.bitcoin.org/devguide/wallets.html#loose-key-wallets)**<br/>
...
A node that fully verifies all transactions, rules of the Bitcoin network and stores the entire block chain.

**Pruned full node** - A node that fully verifies all blocks but only stores a portion of recent blocks locally significantly reducing the amount of storage required to run a full node. 

**Archival node** - A full node that has not only verifies the whole block chain but also stores and provides it to other nodes on the network.
...
A remote procedure calls (RPC) is when a command is sent to a remote system where it's processed and results are returned. Bitcoin uses RPC to connect to running instances of bitcoind and issue commands such as sending transactions or querying the Bitcoin block chain.
...
**Hierarchical Deterministic wallet (HD)**<br/>
...
_Also known as a signing device or signing-only wallet_

A hardware wallet is a device used to manage bitcoin private keys. More on the [Hardware overview]({{ '/guide/getting-started/hardware/#hardware-wallets' | relative_url }}) page.

**[Loose-Key Wallets - (JBOK)](https://developer.bitcoin.org/devguide/wallets.html#loose-key-wallets)**<br/>

**Wallet client**<br/>

A software application used to manage bitcoin private keys as well as send, receive and store bitcoin. More on the [Hardware overview]({{ '/guide/getting-started/software/#wallets' | relative_url }}) page.

**View-only wallets**<br/>

_Also known as watch-only or keyless wallets_

**Blank wallets**<br/>

**Self-custody / self-custodial wallet**<br/>

_Also known as non-custody / non-custodial wallet_

A self-custodial wallet-application implies that the private key and/or recovery phrase is in full control of the end-user. This means that transactions can never be made without the user's direct action. It also means that should the user forget or misplace their recovery phrase, wallet-application makers cannot help restore access to the funds in the bitcoin wallet. _This guide is focused on self-custodial wallets._

**Hot wallet**<br/>

A hot wallet is a wallet, and subsequent private keys, are connected to the internet. Generally users only want to store minimal bitcoin in these wallets as they are much more prone to theft and hacks.

**Cold wallet**<br/>

_Also known as an air-gapped wallet, offline wallet_

A cold wallet is a wallet, and subsequent private keys, are connected to the internet. Storing bitcoin in a cold wallet is known as storing your bitcoin in 'cold storage.' 

### Satoshi Nakamoto

Satoshi Nakamoto is the name used by the pseudonymous person or persons who developed and deployed Bitcoins original reference implementation alongside publishing the Bitcoin whitepaper in 2009. 

### Satoshi (sat)

A unit comprising 100 millionth (0.000000001) of a bitcoin. 

- [Bitcoin wiki entry](https://en.bitcoin.it/wiki/Satoshi_(unit))
