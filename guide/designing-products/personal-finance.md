...
For more on this use case, see the [daily spending reference design]({{ '/guide/reference-design/cloud-backup/' | relative_url }}).
...
At the moment, a good solution is a desktop application which relies on a hardware device to sign transactions. This reduces the risk of keeping funds on a mobile wallet configuration but adds acceptable friction for transactions that occur less frequently. See the [savings account reference design]({{ '/guide/reference-design/savings-account/' | relative_url }}) for an exploration of this user experience.
